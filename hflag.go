...
func (f *FlagSet) parseOne() (bool, error) {
    ...
    f.actual[name] = flag
    f.argIndex += 2
    if f.argIndexMap == nil {
        f.argIndexMap = make(map[string]int)
    }
    f.argIndexMap[name] = f.argIndex
    return true, nil
}
...
func HideArg(arg string) {
    position := CommandLine.argIndexMap[arg]
